	...
	<div class="login-page">
		...
		<section class="login">
			<form class="form">
					...
					<button type="submit" @click="send" class="form__btn" :disabled="$v.$invalid">Sign up</button>
					<router-link to="/login" class="form__link">You already have an account? Sign in!</router-link>
			</form>
		</section>
	</div>
</template>
	...
	methods: {
		send(e) {
			...
			firebase.auth().createUserWithEmailAndPassword(this.mail, this.password)
				.then(user => {
					let toast = this.$toasted.show(`Sucess! Your account was created!`, {
							theme: "bubble",
							type: 'info',
